...
Nine of the ten checks are entirely passive. QA-09 requests 13 well-known paths (plus one calibration probe) on the origin
...
node test/run-tests.mjs       # 65 unit tests — checks, parsers, licence crypto
node test/compat.mjs          # 13 cross-browser manifest + source-portability tests
...
node test/ci.mjs              # 13 CI policy tests (what fails a build, and why)
node test/ci-integration.mjs  # 12 tests running the Action as GitHub invokes it
node test/audit-accuracy.mjs  # diffs the corpus against independently-written logic
```

**136 tests total.** `ext-smoke.mjs` runs the whole scan flow twice: once with a Chromium-style
...
ci/            GitHub Action: input/output plumbing and the build-failing policy
